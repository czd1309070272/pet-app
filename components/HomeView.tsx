...
import { Scan, Search, FileText, Heart, MapPin, ShieldCheck, ChevronRight, Activity, Sparkles, Calendar as CalendarIcon, Pill, Wallet, User, Languages, Crown, Image as ImageIcon, LayoutGrid, MessageSquareText, Stars, BookOpen, Clock, Loader2 } from 'lucide-react';
import { View, DiaryEntry, Medication, Article, url_base } from '../types';
...
const HomeView: React.FC<HomeViewProps> = ({ onNavigate, entries, medications, pets, user }) => {
  ...
  useEffect(() => {
    ...
    backend.fetchArticles().then(setArticles);
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCurrentArticleIdx(prev => (prev + 1) % articles.length);
    }, 3000);
    ...
    <div className="p-6 space-y-8 pb-10">
      ...
      >
        ...
        <div className="flex items-center space-x-5">
          <div className="relative">
            <img src={`${url_base}${activePet.avatar}`} alt="Pet Avatar" className={`w-20 h-20 rounded-3xl border-4 object-cover shadow-xl ${activePet.isMemorial ? 'border-amber-500/30 grayscale' : 'border-white/30'}`} />
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-end px-1">
          <div className="space-y-1">
            <h3 className="text-base font-black text-gray-800 dark:text-white flex items-center tracking-tight">
              <BookOpen size={18} className="mr-2 text-orange-500" />
              萌寵百科指南
            </h3>
            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Expert Insight Slideshow</p>
          </div>
          <button
            onClick={() => onNavigate(View.ARTICLE_LIST)}
        ...
        <div className="relative overflow-hidden rounded-[32px] aspect-[16/9] shadow-xl border border-white/60 dark:border-white/5 bg-white dark:bg-slate-900 group">
          {articles.length > 0 ? (
            <div
              className="flex transition-transform duration-1000 ease-in-out h-full"
              style={{ transform: `translateX(-${currentArticleIdx * 100}%)` }}
            >
              {articles.map(article => (
                <div
                  key={article.id}
                  onClick={() => onNavigate(View.ARTICLE_DETAIL, article.id)}
                  className="w-full flex-shrink-0 relative h-full cursor-pointer overflow-hidden"
                >
                  <img src={article.coverImage} className="w-full h-full object-cover transition-transform duration-[3s] group-hover:scale-105" alt={article.title} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent"></div>
                  <div className="absolute bottom-0 left-0 right-0 p-6 space-y-2">
                    <div className="flex items-center space-x-2">
                      <span className="bg-orange-500 text-white text-[8px] font-black px-2 py-0.5 rounded-md uppercase tracking-tighter shadow-lg">
                        {article.category}
                      </span>
                      <span className="text-white/60 text-[8px] font-bold flex items-center">
                        <Clock size={10} className="mr-1" /> {article.readTime}
                      </span>
                    </div>
                    <h4 className="text-lg font-black text-white leading-tight line-clamp-1 drop-shadow-md">
                      {article.title}
                    </h4>
                    <p className="text-xs text-white/70 font-medium line-clamp-1 opacity-90 drop-shadow-sm">
                      {article.summary}
                    </p>
                  </div>
                </div>
            ...
            <div className="w-full h-full flex flex-col items-center justify-center opacity-20">
              ...
              <span className="text-xs font-black">正在編撰百科...</span>
            </div>
          )}
          <div className="absolute top-4 right-6 flex space-x-1.5 bg-black/20 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
            {articles.map((_, i) => (
              <div
                key={i}
                className={`h-1 rounded-full transition-all duration-500 ${currentArticleIdx === i ? 'bg-orange-500 w-4' : 'bg-white/30 w-1'}`}
              />
            ))}
          </div>
        </div>
                ...
                {todayMeds.length > 0 ? (
                  ...
                  warmPhrases.map((phrase, i) => (
                    <div key={i} className="h-6 flex items-center w-full min-w-0">
                      <p className="text-[10px] font-black text-gray-400 dark:text-gray-400 leading-tight truncate w-full">{phrase}</p>
                    </div>
                  ))
                )}
              </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <button onClick={() => onNavigate(View.AI_CONSULTANT)} className="glass p-6 text-left flex flex-col justify-between min-h-[160px] rounded-[36px] shadow-glass border-white/60 clickable bg-purple-50/10">
          ...
          <div>
            <h3 className="font-black text-gray-800 dark:text-slate-100 text-xl leading-tight">AI 顧問</h3>
