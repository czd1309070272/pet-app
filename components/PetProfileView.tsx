...
import { url_base, WeightEntry } from '../types';
...
const PetProfileView: React.FC<PetProfileViewProps> = ({ onBack, initialPet }) => {
  ...
  useEffect(() => {
    console.log('PetProfileView useEffect triggered, initialPet:', initialPet);
    if (initialPet) {
      const loadProfile = async () => {
        console.log('Loading pet profile...');
        const profile = await backend.fetchPetProfile(initialPet.id);
      ...
      const loadWeight = async () => {
        try {
          console.log('Loading weight history for pet ID:', initialPet?.id);
          const history = await backend.fetchWeightHistory(initialPet.id); // 👈 传 pet.id
          console.log('Fetched weight history:', history);
      ...
      console.log('Setting initial pet info:', initialPet);
  ...
  const handleUpdateWeight = async () => {
    ...
    try {
      const updatedHistory = await backend.updatePetWeight(newWeight, initialPet.id);
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[40px] p-6 shadow-xl border border-gray-100 dark:border-white/5 space-y-6">
            <div className="flex items-center space-x-5">
              <div className="relative group">
                <img src={`${url_base}${petInfo.avatar}`} alt={petInfo.name} className="w-24 h-24 rounded-[32px] object-cover shadow-lg border-4 border-white transition-transform group-active:scale-95" />
                      ...
                      <div className="relative group">
                        <span
                          ...
                          style={{
                            ...
                            maxWidth: '200px', // 根据需要调整宽度
                            maxHeight: '4em', // 控制最大高度，适应多行文本
                            overflow: 'hidden',
                        ...
                        {/* 当鼠标悬停时显示完整爱好 */}
                        {petInfo.hobbies.length > 50 && (
                          <span
                            className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 hidden group-hover:block text-white p-2 overflow-auto"
