...
const MembershipView: React.FC<MembershipViewProps> = ({ onBack, user, onUpdateUser }) => {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      const success = await backend.open_vip(
        user.id,
        selectedPlan,
        selectedMethod
      );
      ...
      if (success) {
        ...
        if (updatedUser) {
          ...
          throw new Error('获取更新后用户信息失败');
        }
      } else {
        // 后端返回 false，视为业务失败（如余额不足、重复购买等）
        throw new Error('支付处理失败，请稍后重试');
      }
    } catch (error: any) {
      console.error('VIP 开通失败:', error);
      setPaymentStatus('FAILED');
      const msg = error?.message || '网络异常，请检查后重试';
      setErrorMessage(msg);
