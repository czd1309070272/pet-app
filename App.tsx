...
const App: React.FC = () => {
    ...
    const initApp = async () => {
      ...
      try {
        ...
        if (currentUser) {
          setUser(currentUser);
          setIsLoggedIn(true);
          const [initialPets, initialEntries, initialMeds, initialAppts, initialExpenses] = await Promise.all([
            backend.fetchPets(),
          ...
          // 关闭公告（可选）
          // setTimeout(() => setIsNoticeOpen(false), 1000);
        } else {
          // ⚠️ 用户信息为空 → 强制登出
          performLogout();
        }
  ...
  const handleNavigate = (view: View, data?: any) => {
    ...
    if ((view === View.PRODUCT_DETAIL || view === View.ARTICLE_DETAIL) && data) {
      setSelectedId(data);
    }
  ...
  const handleBackWithAnimation = () => {
    ...
    setTimeout(async () => {
      ...
      // 刷新用户状态
      const freshUser = await backend.getCurrentUser();
      if (freshUser) {
        setUser(freshUser);
      } else {
        // ⚠️ 用户已失效 → 强制登出
  ...
  const renderView = (view: View, isOverlay: boolean = false) => {
    ...
    switch (view) {
      ...
      case View.ARTICLE_DETAIL: return <ArticleDetailView articleId={selectedId!} onBack={backFn} />;
      case View.ARTICLE_LIST: return <ArticleListView onBack={backFn} onNavigate={handleNavigate} />;
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-md mx-auto relative shadow-2xl transition-colors duration-500 bg-transparent overflow-hidden">
      {!isLoading && ![View.PROFILE, View.SETTINGS, View.HEALTH_SCAN, View.MAP, View.CALENDAR, View.PET_PROFILE, View.HISTORY_HEALTH, View.HISTORY_SCANNER, View.HISTORY_TRANSLATOR, View.WALLET, View.MEDICATION, View.APPOINTMENT, View.INSURANCE, View.MEMBERSHIP, View.ALBUM, View.AI_CONSULTANT, View.PRODUCT_DETAIL, View.CART, View.COMMUNITY_HISTORY, View.ORDERS, View.LOGISTICS, View.ADDRESS, View.PERSONAL_INFO, View.STARRY_MEMORIAL, View.ARTICLE_DETAIL, View.ARTICLE_LIST].includes(currentView) && (
