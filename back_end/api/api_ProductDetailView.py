...
async def add_discoveryview_cart(request_data: DiscoveryBuildCarts):
    try:
        ...
        if not product_id or num <= 0:
            return JsonTool(code=400, msg="商品ID或数量无效", data=None)
        ...
        if existing_cart:
            new_quantity = existing_cart['quantity'] + num
            db.execute(
            ...
            insertSql = """
                ...
                VALUES (%s, %s, %s, NOW())
            """
            insertParam = (product_id, user_id, num)
...
async def get_discoveryview_productmes(request_data: DiscoveryShopMes):
    try:
        # 从 token 解析用户（即使商品公开，也用于后续扩展如收藏/浏览记录）
        decoded = decode_token_for_auth(request_data.token)
        user_id = decoded["user_id"]
        claimed_token_version = decoded["token_version"]
        ...
        product_id = request_data.product_id
        if not product_id:
            return JsonTool(code=400, msg="商品ID不能为空", data=None)
        ...
        product_obj = Product(
            id=str(product_result.get('id', '')),
            product_id=str(product_result.get('product_id', '')),
            name=product_result.get('name', '') or '',
            price=float(product_result.get('price', 0.0)) if product_result.get('price') is not None else 0.0,
            original_price=float(product_result.get('original_price', product_result.get('price', 0.0))) 
                if product_result.get('original_price') is not None 
                else float(product_result.get('price', 0.0)),
            imageUrl=product_result.get('main_image', '') or '',
            category=product_result.get('category', '') or '',
            tags=product_result.get('tags', product_result.get('tag', '')) or '',
            rating=float(product_result.get('rating', 0)) if product_result.get('rating') is not None else 0.0,
            sales=int(product_result.get('sales', 0)) if product_result.get('sales') is not None else 0,
            description=product_result.get('description', '') or ''
        )
